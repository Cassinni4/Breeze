...
const encodedGatewayFile = 'gateway_encoded.txt';
...
function encryptGateway() {
    var fields;

    fs.readFile(decodedGatewayFile, 'utf8', (err, data) => {
        if (err) {
            console.error(err);
            return;
        }
        ...
        fields.forEach(element => {
            ...
            if ('{' != element) {
                var input = element.substring(1,element.length-1); // ignore quotes
                var encrypted = logResult(encrypt(input)); // Encrypt the data
                decryptVal[counter] = input;
                encryptVal[counter] = encrypted;
            } else {
                index++;
            }
        });
        ...
        for (let index = 0; index <= counter; index++) {
            ...
            logIt(decryptVal[index] + " = " + encryptVal[index], 0);
            outdata = outdata.replaceAll(decryptVal[index], encryptVal[index]);    
        }

        fs.writeFile(encodedGatewayFile, outdata, err => {
            if (err) {
                console.error(err);
                return;
            }
...
function encrypt(plaintext) {
    ...
    //var indata = CryptoJS.enc.Hex.parse("2" + "SPD_Spiderman_PS4"); // example string captured during exe debug
    try encrypt
    var indata = CryptoJS.enc.Hex.parse("2" + "SPD_Spiderman_PS4"); // just strings for now could probably infer type from key
